...
public class EstiloDAO {
	...
	public static boolean save(basededatos.Estilo estilo) throws PersistentException {
		try {
			AplicacióndeBúsquedayReproduccióndeMúsicaPersistentManager.instance().saveObject(estilo);
			return true;
		}
		catch (Exception e) {
			e.printStackTrace();
	...
	public static boolean delete(basededatos.Estilo estilo) throws PersistentException {
		try {
			AplicacióndeBúsquedayReproduccióndeMúsicaPersistentManager.instance().deleteObject(estilo);
			return true;
		}
		catch (Exception e) {
			e.printStackTrace();
	...
	public static boolean deleteAndDissociate(basededatos.Estilo estilo)throws PersistentException {
		try {
			basededatos.Estadistica[] lAparece_ens = estilo.aparece_en.toArray();
			for(int i = 0; i < lAparece_ens.length; i++) {
				lAparece_ens[i].estilos_mas_escuchados.remove(estilo);
			}
			basededatos.Artista[] lArtistass = estilo.artistas.toArray();
			for(int i = 0; i < lArtistass.length; i++) {
				lArtistass[i].setEstilo(null);
			}
			basededatos.Cancion[] lCancioness = estilo.canciones.toArray();
			for(int i = 0; i < lCancioness.length; i++) {
				lCancioness[i].estilos.remove(estilo);
			}
	...
	public static boolean refresh(basededatos.Estilo estilo) throws PersistentException {
		try {
			AplicacióndeBúsquedayReproduccióndeMúsicaPersistentManager.instance().getSession().refresh(estilo);
			return true;
		}
		catch (Exception e) {
			e.printStackTrace();
	...
	public static boolean evict(basededatos.Estilo estilo) throws PersistentException {
		try {
			AplicacióndeBúsquedayReproduccióndeMúsicaPersistentManager.instance().getSession().evict(estilo);
			return true;
		}
		catch (Exception e) {
			e.printStackTrace();
